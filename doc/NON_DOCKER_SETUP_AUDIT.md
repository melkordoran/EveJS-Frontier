...
### 1. Server startup is coupled to client setup — FIXED

`StartServer.bat` resolved and loaded the client-generated `EvEJSConfig.bat` before it checked server dependencies or data. As a result, even **Server only** could not start until the client wizard had run. The Node server itself uses root JSON and `EVEJS_*` environment variables, so this was a launcher coupling rather than a runtime requirement.

This half of a genuine circular dependency: the launcher that installs the Node packages could not run until the client wizard had run, and the client wizard's certificate step could not run until the Node packages were installed (`build-gateway-cert.js` needs `node-forge`).

**Fixed.** `ResolveConfigDir` now falls back to `EvEJSConfig.example.bat` and never fails; a real client configuration is required only when the user chooses **Server + Play**, which falls back to server-only when no client is set up. The other half was fixed by shipping `node-forge` in the root `package.json` and having `Install-EvEJSCerts.ps1` install the root packages itself when it needs them.
...
- ~~database-creator errors mention a nonexistent `SetupEveJS.bat`~~ — `SetupEveJS.bat` now exists at the project root, so this reference is correct.
...
### P1: create one backend workflow — PARTLY DONE
...
`SetupEveJS.bat` (`tools\Setup\Setup-EvEJS.ps1`) now covers the setup and doctor
halves of this for both native and Docker installs:

```text
SetupEveJS.bat                  run every phase that is not already done
SetupEveJS.bat -Status          report state, change nothing (the doctor role)
SetupEveJS.bat -Only <phase>    run a single phase
SetupEveJS.bat -SkipClient      server-only host, no EVE client
SetupEveJS.bat -NonInteractive  no prompts, for scripted installs
```

Every phase derives its state from the filesystem rather than a progress file,
so the script is safe to interrupt and re-run. Still outstanding: the `start:*`
commands, and unifying the market/server runtime launchers.

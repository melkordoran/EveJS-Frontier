...
struct SeederConfig {
    ...
    build: BuildConfig,
    #[serde(default)]
    custom_station: Option<CustomStationConfig>,
}

/// Seeds one buy + one sell order for every reference-priced item at a chosen station,
/// copying the reference system's (Jita) top-of-book price at the configured quantity.
#[derive(Debug, Clone, Deserialize)]
struct CustomStationConfig {
    station_id: u64,
    #[serde(default = "default_reference_solar_system_id")]
    reference_solar_system_id: u32,
    #[serde(default = "default_custom_station_quantity")]
    quantity: u64,
}
...
fn default_custom_station_quantity() -> u64 {
    1_000_000
}
...
struct ImportStats {
    ...
    seed_sell_rows: u64,
    seed_buy_rows: u64,
    custom_station_sell_rows: u64,
    custom_station_buy_rows: u64,
...
fn build_database(config: &SeederConfig, args: BuildArgs) -> Result<()> {
    ...
    let import_stats = import_station_orders(
        ...
        &market_solar_system_ids,
        npc_order_duration_threshold_days,
        config.custom_station.as_ref(),
        config
            .custom_station
            .as_ref()
            .map(|custom| custom.reference_solar_system_id)
            .unwrap_or(JITA_SYSTEM_ID),
    )?;
...
fn import_station_orders(
    ...
    // Best sell (min) / buy (max) price in cents at the reference system (Jita), keyed by
    // type, used to seed the custom station at reference prices.
    let mut ref_sell = BTreeMap::<u32, i64>::new();
    let mut ref_buy = BTreeMap::<u32, i64>::new();
    ...
    for result in reader.deserialize::<EveRefOrderRow>() {
        ...
        // Track reference-system (Jita) top-of-book so the custom station can copy it.
        if system_id == reference_solar_system_id {
            if row.is_buy_order {
                ref_buy
                    .entry(row.type_id)
                    .and_modify(|best| {
    ...
    if let Some(custom) = custom_station {
        let (sell_rows, buy_rows) =
            write_custom_station(connection, static_data, custom, &ref_sell, &ref_buy)?;
        stats.custom_station_sell_rows = sell_rows;
        stats.custom_station_buy_rows = buy_rows;
    }
    ...
    spinner.finish_with_message(format!(
        ...
        format_count(stats.station_orders),
        format_count(
            stats.seed_sell_rows
                + stats.seed_buy_rows
                + stats.custom_station_sell_rows
                + stats.custom_station_buy_rows
        )
    ));
    Ok(stats)
}
...
/// Seeds one sell + one buy order per reference-priced item at a custom station, copying the
/// reference system's (Jita) top-of-book price at the configured quantity. Written to the seed
/// tables so it flows through the normal per-region order book + summary + seed-fill path.
/// `INSERT OR REPLACE` overrides any existing NPC seed rows at this station (PK station_id,type_id).
fn write_custom_station(
    connection: &mut Connection,
    static_data: &StaticData,
    custom: &CustomStationConfig,
    ref_sell: &BTreeMap<u32, i64>,
    ref_buy: &BTreeMap<u32, i64>,
) -> Result<(u64, u64)> {
    let Some(station) = static_data
        .stations
        .iter()
        .find(|station| station.station_id == custom.station_id)
    else {
        println!(
            "{} custom_station {} not found in static data; skipping",
    ...
    let quantity = sqlite_quantity(custom.quantity);
    let seeded_at = now_rfc3339();
    let mut sell_rows = 0u64;
    let mut buy_rows = 0u64;
    let transaction = connection.transaction()?;
    {
        let mut sell_stmt = transaction.prepare(
            "INSERT OR REPLACE INTO seed_stock (
               station_id, solar_system_id, constellation_id, region_id,
               type_id, price, quantity, initial_quantity, updated_at
             ) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?7, ?8)",
        )?;
        for (&type_id, &price_cents) in ref_sell {
            if price_cents <= 0 {
                continue;
            }
            sell_stmt.execute(params![
                station.station_id,
                station.solar_system_id,
                station.constellation_id,
                station.region_id,
        ...
        let mut buy_stmt = transaction.prepare(
            "INSERT OR REPLACE INTO seed_buy_orders (
               station_id, solar_system_id, constellation_id, region_id,
               type_id, price, quantity, initial_quantity, updated_at
             ) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?7, ?8)",
        )?;
        for (&type_id, &price_cents) in ref_buy {
            if price_cents <= 0 {
                continue;
            }
            buy_stmt.execute(params![
                station.station_id,
                station.solar_system_id,
                station.constellation_id,
                station.region_id,
    ...
    transaction.commit()?;
    println!(
        "{} seeded {} sell + {} buy orders at {} (station {}) at reference prices, qty {}",
        style("[custom-station]").cyan().bold(),
        format_count(sell_rows),
        format_count(buy_rows),
        station.station_name,
        station.station_id,
        format_count(custom.quantity)
    );
    Ok((sell_rows, buy_rows))
}
